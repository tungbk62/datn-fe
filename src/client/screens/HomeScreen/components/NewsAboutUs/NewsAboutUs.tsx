...
import { BaseLazySkeletonImage } from "@components-client/BaseLazySkeletonImage";
import { BaseTextBoxSlice } from "@components-client/BaseTextBoxSlice";
import Skeleton from "react-loading-skeleton";
import { useMediaQuery, useTheme } from "@material-ui/core";
import { Carousel } from "antd";
import { CarouselItem } from "@components-client/AuthWrapper/components/CarouselItem";
...
const NewsAboutUs = (props: Props): JSX.Element => {
  ...
  return (
    <div className={classes.categoryContainer}>
      ...
      <div 
      className={classes.swiperCategory}
      >
        ...
        <Grid className={classes.box} item xs={4}>
          {data?.length && data?.map((item:any,index:number)=>{
            if (index <= 7) {
              return (<div onClick={()=>{onPressPost(item)}} className={classes.categoryName} key={index}>{item?.title}</div>)
            }
            return null;
          })}
        </Grid>
